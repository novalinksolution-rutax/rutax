...
- `(consola)/` — tableros a **pantalla completa**, fuera del `AppShell`: sin sidebar, sin `max-w`, viewport fijo. Mismos guards de acceso que `(tenant)`. Hoy solo la Torre de control. No es donde van las pantallas nuevas del courier — esas siguen en `(tenant)`.
...
## Torre de control — en rediseño v2 (2026-08-03)

El módulo **Torre de control** es la capa de anticipación operativa del courier. Vive en `src/app/(consola)/torre-de-control/` (UI) y `src/modules/contexto/` (motor y composer), con sus adaptadores en `src/modules/integraciones/contexto/`.

**El handoff de diseño dejó de mandar.** `design_handoff_torre_de_control/` era la interfaz aprobada y la referencia obligatoria; el usuario decidió rediseñar el módulo, así que sus 7 reglas de producto, sus 6 regiones, sus tokens y sus copys ya **no** son autoridad. Todo eso está archivado en `docs/_historico/torre-v1/` — se conserva porque explica por qué el código de hoy es como es, no para seguirlo. Con él cae el "contrato congelado" de tipos: `src/modules/contexto/contrato-torre.ts` pasa a ser un tipo **vivo y editable**.

Decisión de producto ya tomada: **el mapa es exclusivamente operativo** — zonas, conductores, pedidos y carga. Nada de clima, lluvia ni tránsito: eso se ve desde el teléfono del conductor y en terreno.

Documentos que sí mandan:
- `@docs/torre-de-control/alcance-v2.md` — qué hace la Torre v2, qué se retira y por qué. *(En definición en esta pasada; hasta que exista, no hay fuente de verdad de producto para el módulo — pregunta antes de asumir.)*
- `@docs/arquitectura/torre-de-control.md` — diseño técnico: esquema `contexto` con su carve-out deny-all, puertos de integración, jobs y cadencias, motor de riesgo, pipeline de cartografía. Sus secciones de pantalla y de fuentes están marcadas como superadas.
...
- **§4 del diseño técnico se equivocaba con las fuentes.** Open-Meteo **no sirve**: su tier libre prohíbe uso comercial y define como comercial "apps con suscripciones". **Migrado (2026-07-27): clima Y aire van con OpenWeather** — el tier gratuito no pide tarjeta, permite uso comercial y a cambio exige **atribución visible en pantalla** con el texto literal «Weather data provided by OpenWeather» (está al pie del mapa; quitarla incumple la licencia). Clima usa `/data/2.5/forecast`, con paso de **3 horas** —el gratuito no da hora a hora— y `rain.3h` es acumulado, no intensidad. Aire usa Air Pollution API (horario, 4 días) y siembra la media móvil de 24 h con su endpoint de histórico. **MMA/SINCA quedó descartado como fuente del factor aire: publica observaciones, no pronóstico** — el mismo defecto por el que se descartó la DMC para clima; sigue siendo la fuente correcta para "qué mide la ciudad ahora" y sería un adaptador nuevo detrás del mismo puerto. Muestrear las 52 comunas es sobre-muestrear: van **14 puntos de grilla** (`src/modules/integraciones/contexto/grilla-rm.ts`, k-centros sobre los centroides reales, peor caso 12,6 km) y cada comuna toma el más cercano. Consecuencia asumida: un solo punto cubre las 25 comunas del casco urbano, así que clima y aire no distinguen Centro de Oriente — el modelo tampoco.
- **Umbrales PM2.5 reales** (Plan Operacional GEC 2026 del MMA): Alerta 80 · Preemergencia 110 · Emergencia 170, sobre la **media móvil de 24 h**, no sobre la hora suelta. Los del dummy archivado (`docs/_historico/torre-v1/datos-dummy.ts`) están mal — nunca fue contrato de valores.
- **`EstadoTorre` se envuelve** en `TorreRespuesta { horizonteInicial, horizontes: Record<'hoy'|'manana'|'72h', EstadoTorre> }`. `olas` no va ahí. **A 72 h se cuentan solo pedidos ya ingestados, nunca una proyección**: se verá casi vacío, y es correcto. *(Cuántos horizontes quedan es pregunta abierta del rediseño v2.)*
...
- **La Torre NO vive en `(tenant)`: vive en `src/app/(consola)/`**, un grupo de rutas nuevo (sexto destino de `src/app/`). Decisión del usuario, por fidelidad al handoff: es un tablero de viewport fijo donde solo el riel scrollea, y eso es incompatible con el `AppShell` (sidebar + `<main>` con `max-w-6xl` + scroll de página). `(consola)/layout.tsx` repite **exactamente** los mismos guards de sesión y tipo de usuario que `(tenant)/layout.tsx`; salir del shell no es salir del control de acceso. **La regla general no cambia: toda pantalla nueva del courier sigue yendo a `(tenant)`.** `(consola)` es para tableros a pantalla completa, y hoy solo lo usa la Torre.
