...
- **Retiro en bodega del seller + ruteo del conductor** — **ENTRA (decisión del usuario, 2026-08-11)**, a partir de una reunión con un courier real. Son dos piezas de un mismo flujo: el conductor va a la bodega del seller, retira, y sale a repartir. **Lee `@docs/arquitectura/retiro-y-ruteo.md` antes de tocar este alcance — es la fuente de verdad.** Resumen v1:
  ...
  - 🚨 **BLOQUEADOR RAÍZ, VERIFICADO EN CÓDIGO (2026-08-12): Rutax NO ingesta los pedidos Flex del día.** Solo dos lugares insertan en `operacion.pedidos`: `src/modules/operacion/pedidos.ts` (same-day) y `ejecutar-backfill.ts` (Flex). El backfill se dispara con **un único evento**, `ml/conexion.reconectada`, publicado solo desde `puerto.ts:303` en el intercambio OAuth, con ventana de 7 días. El webhook ignora en silencio el shipment que no está en BD (`procesar-shipment.ts:161-168`) y el polling solo recorre pedidos ya existentes en `asignado`/`en_ruta`. Ninguno de los tres crons de ML ingesta. **Un pedido Flex nuevo no entra al sistema.** Además `estado_ml` guarda `order.status` y no el estado del envío (`ejecutar-backfill.ts:420`), así que `ready_to_ship` —lo que diría qué hay listo para retirar— no está en ninguna columna; y `destinatario_nombre` se llena con el título del producto (`:422`). **Todo el alcance de retiro y ruteo se apoya en esta tabla: se arregla esto primero o lo demás cuenta aire.**
  - **Bloqueadores conocidos, a resolver dentro de este alcance:** (1) `identidad.sellers` **no tiene dirección ni bodega** — sin eso el retiro no tiene lugar físico ni la ruta punto de partida; y un seller puede tener **varias** bodegas. (2) El **tope de cuentas ML por seller sube de 3 a 10** (decisión del usuario, 2026-08-12): la realidad ya desmintió el 3 — este courier tiene un seller con 4 cuentas vinculadas. Hay que tocar el trigger, la validación en TypeScript, los tests y los textos de interfaz que digan "3". (3) `operacion.orden_paradas` ordena **alfabéticamente** por comuna y dirección, y no se persiste en ninguna columna.
