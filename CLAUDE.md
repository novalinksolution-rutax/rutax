...
El módulo **Torre de control** es la consola de monitoreo del día del courier. Vive en `src/app/(consola)/torre-de-control/` (UI) y `src/modules/contexto/` (composer y agregación), con sus adaptadores en `src/modules/integraciones/contexto/`.
...
⚠️ Mostrar la **dirección del destinatario** en el punto del mapa (F3) **no se construye sin revisión previa de `seguridad-cumplimiento`**. Y no se guarda recorrido del conductor: sigue habiendo una sola fila por conductor, la última posición, sin histórico (Ley 21.431).
