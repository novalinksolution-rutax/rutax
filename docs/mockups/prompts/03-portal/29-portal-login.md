...
VISTA: `/portal/login` — Inicio de sesión del seller
DISPOSITIVO: escritorio, formulario centrado (16:10)
CONTEXTO: Acceso del seller a su portal. Usa el mismo formulario central de login de Rutax (login propio del seller).
...
- Campo "Correo electrónico": etiqueta arriba, input con ícono de sobre, placeholder `tu@correo.cl`.
- Campo "Contraseña": etiqueta arriba, input con ícono de candado, valor enmascarado `••••••••`.
- Botón primario navy de ancho completo "Ingresar".
DATOS DE EJEMPLO (ilustrativos): seller "Tienda Aurora"; correo `paula@boutiquelila.cl`.
NOTA DE CONVERGENCIA: idéntico al login interno `/login` y a la entrada `/` — misma tarjeta, inputs con ícono y botón navy. Mantén EXACTAMENTE tipografía y paleta del set.
