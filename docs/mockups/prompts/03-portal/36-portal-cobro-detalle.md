...
VISTA: `/portal/cobros/[id]` — Detalle de cobro + descargar factura (seller)
DISPOSITIVO: escritorio 16:10
CONTEXTO: Detalle de un período del seller con su factura descargable y el detalle de entregas. Transparente y sin sorpresas; sin monto base ni ajustes internos (solo el monto final por entrega).
ESTRUCTURA (portal del seller): cabecera superior blanca liviana con "Portal del seller" + nav horizontal + "Cerrar sesión". Contenido centrado (~960px) con aire, sobre gris claro.
LAYOUT del contenido:
- Migas de pan: "Estado de cuenta / Detalle".
- Encabezado: rango de fechas en gris, fila de badges (verde "Facturado — Folio 1042" + verde "Pagado"), y el **monto total muy grande** en CLP ($ 1.240.500). Si está pagado, línea verde "Pago recibido. Gracias."
- Sección "TU FACTURA": tarjeta blanca con "Folio 1042" grande, "Emitida el 15/06/2026", el monto total en CLP, un badge de estado SII con ícono (verde-check "Aceptado por SII"), y un botón primario navy GRANDE y evidente "Descargar factura (PDF)".
- Sección "DETALLE DE ENTREGAS (24 líneas)": tabla espaciada. Columnas: **Pedido** (id mono), **Fecha entrega**, **Concepto**, **Monto** (derecha, negrita CLP). Fila de pie "Total" con la suma en CLP.
DATOS DE EJEMPLO (ilustrativos): seller "Tienda Aurora"; período 01/06/2026 – 15/06/2026; conceptos "Entrega — Maipú", "Entrega — Ñuñoa".
NOTA DE CONVERGENCIA: la descarga de factura es una acción grande y evidente (botón navy); badges SII/período/cobro y montos CLP del set; densidad relajada del portal. Header del portal idéntico al resto.
