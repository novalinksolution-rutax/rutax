...
VISTA: `/portal` — Inicio del portal del seller (estado de conexión ML)
DISPOSITIVO: escritorio 16:10
CONTEXTO: Home del seller. Responde de un vistazo "¿está todo bien con mi conexión a Mercado Libre?" en lenguaje humano, con la única acción "Reconectar" cuando hace falta. Experiencia más espaciada y tranquilizadora que el backoffice.
ESTRUCTURA (portal del seller, idéntica en todas las vistas `portal/`): cabecera superior blanca fina y LIVIANA (no hay barra lateral) con, a la izquierda, el texto "Portal del seller", y a la derecha una navegación horizontal sobria: "Inicio" (activa), "Mis pedidos", "Estado de cuenta", "Incidencias", y un botón discreto "Cerrar sesión". Contenido centrado en una columna de ancho máx ~960px sobre fondo gris claro, con bastante aire.
LAYOUT del contenido:
- Título "Mi portal" (semibold ~24px) y subtítulo gris "El estado de tu conexión con Mercado Libre, de un vistazo."
- Tarjeta grande de estado de conexión: ícono de estado (círculo verde con check), título grande "Conexión sana", texto gris tranquilizador "Tus pedidos llegan automáticamente desde Mercado Libre." y, debajo, un badge verde "Conectado". Si estuviera caída, el ícono/badge serían rojos y aparecería un botón primario navy "Reconectar".
DATOS DE EJEMPLO (ilustrativos): seller "Tienda Aurora SpA"; cuenta ML conectada.
NOTA DE CONVERGENCIA: el header liviano del portal (logo/título + nav horizontal, ancho ~960px) debe verse idéntico en todas las vistas `portal/`. Más aire y densidad relajada que el backoffice; badges y botón navy del set.
