...
VISTA: `/portal/pedidos` — Mis pedidos (tracking del seller)
DISPOSITIVO: escritorio 16:10
CONTEXTO: El seller sigue sus envíos. Solo lectura, lenguaje de seller (no de courier). Tabla espaciada (densidad relajada) con filtros y paginación.
ESTRUCTURA (portal del seller): cabecera superior blanca liviana con "Portal del seller" a la izquierda y nav horizontal ("Inicio", "Mis pedidos" activa, "Estado de cuenta", "Incidencias") + "Cerrar sesión" a la derecha. Contenido centrado (~960px) con aire, sobre gris claro.
LAYOUT del contenido:
- Encabezado: título "Mis pedidos" (semibold ~24px) y subtítulo gris "Seguimiento de tus entregas. Los estados se actualizan automáticamente."; a la derecha botón primario navy con "+" "Solicitar envío same-day".
- Barra de filtros sencilla: selector "Estado" y campo de fecha.
- Tabla espaciada (densidad relajada) en tarjeta, con conteo gris "42 pedidos". Columnas: **Estado** (badge), **Destinatario** (negrita + comuna gris debajo), **Dirección**, **F. compromiso**.
DATOS DE EJEMPLO (ilustrativos): destinatarios "Valentina Muñoz" (Maipú), "Joaquín Tapia" (Las Condes), "Antonia Reyes" (Ñuñoa); estados Entregado/En ruta/Pendiente de asignación.
NOTA DE CONVERGENCIA: misma tabla y badges del set, pero con densidad relajada (filas más altas, más aire) propia del portal. Header del portal idéntico al resto.
