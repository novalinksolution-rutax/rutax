...
VISTA: `/portal/conectar-ml` — Conectar / reconectar Mercado Libre (OAuth)
DISPOSITIVO: escritorio 16:10
CONTEXTO: Paso de conexión OAuth del seller con Mercado Libre. Una sola acción dominante; tras volver, muestra el resultado (éxito o error). Vista en su variante CTA inicial.
ESTRUCTURA (portal del seller): cabecera superior blanca liviana con "Portal del seller" a la izquierda y nav horizontal + "Cerrar sesión" a la derecha. Contenido centrado (~640px) con aire, sobre gris claro.
LAYOUT del contenido (bloque centrado):
- Tarjeta blanca central con el logo/ícono de Mercado Libre o un ícono de enlace, título "Conecta tu cuenta de Mercado Libre" (semibold), y un párrafo gris explicando que conectar su cuenta principal hace que sus pedidos lleguen solos.
- Instrucción destacada (recuadro tenue) recordando usar la **cuenta principal** de Mercado Libre.
- Botón primario navy grande "Conectar con Mercado Libre".
- (Variante resultado, referencia: tras volver, la tarjeta muestra un ícono de check verde y "Conexión exitosa — tus pedidos llegan automáticamente", o un ícono rojo con un mensaje de error y botón "Reintentar".)
DATOS DE EJEMPLO (ilustrativos): seller "Tienda Aurora".
NOTA DE CONVERGENCIA: header del portal idéntico; una sola acción primaria navy; mensajes de resultado con los colores de estado del set (verde éxito / rojo error). Densidad relajada del portal.
