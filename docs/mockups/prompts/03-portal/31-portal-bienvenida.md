...
VISTA: `/portal/bienvenida` — Onboarding del seller (bienvenida)
DISPOSITIVO: escritorio 16:10
CONTEXTO: Primera pantalla del seller tras aceptar la invitación. Explica qué gana (tracking, estado de cuenta, incidencias) y anticipa el paso de conectar Mercado Libre. Una sola variante informativa, botón único "Continuar".
ESTRUCTURA (portal del seller): cabecera superior blanca liviana con "Portal del seller" a la izquierda y nav horizontal ("Inicio", "Mis pedidos", "Estado de cuenta", "Incidencias") + "Cerrar sesión" a la derecha. Contenido centrado (~640px), MUY centrado y con aire, sobre gris claro.
LAYOUT del contenido (bloque centrado vertical):
- Círculo de fondo navy muy tenue con un ícono de check navy.
- Título grande (semibold ~24px): "Flex Couriers SpA te invitó a su portal de despachos".
- Párrafo gris de bienvenida dirigido al seller.
- Tarjeta blanca con tres filas, cada una con un ícono navy de línea y una frase: "Sigue el estado de tus envíos en tiempo real…", "Revisa tu estado de cuenta y tus documentos…", "Reporta incidencias directo desde aquí…".
- Recuadro gris tenue: "**Antes de empezar:** para sincronizar tus pedidos vamos a necesitar que conectes tu cuenta de Mercado Libre. En el siguiente paso te explicamos cómo hacerlo."
- Botón primario navy "Continuar".
DATOS DE EJEMPLO (ilustrativos): courier "Flex Couriers SpA"; seller "Tienda Aurora".
NOTA DE CONVERGENCIA: header del portal idéntico al resto; ícono en círculo navy-tenue como en las pantallas de activación; mucho aire (densidad relajada). Tipografía y paleta del set.
