...
VISTA: `/portal/incidencias` — Mis incidencias (seller)
DISPOSITIVO: escritorio 16:10
CONTEXTO: El seller ve las incidencias registradas en sus pedidos. Solo lectura, sin acciones de cambio de estado. Si no hay ninguna, estado de buen-estado tranquilizador.
ESTRUCTURA (portal del seller): cabecera superior blanca liviana con "Portal del seller" + nav horizontal ("Inicio", "Mis pedidos", "Estado de cuenta", "Incidencias" activa) + "Cerrar sesión". Contenido centrado (~960px) con aire, sobre gris claro.
LAYOUT del contenido:
- Título "Mis incidencias" (semibold ~24px) y subtítulo gris "Incidencias registradas en tus pedidos. Solo lectura."
- Barra de filtros sencilla: selectores "Tipo" y "Estado".
- Tabla espaciada (densidad relajada) en tarjeta, con conteo gris. Columnas: **Estado** (badge de estado + si supera 4h un badge rojo "Sin gestión: 6h"), **Tipo** (negrita + descripción gris debajo), **Pedido** (id mono), **Abierta hace** (ej. "6h", en rojo si sin gestión).
- VARIANTE alternativa (referencia, sin incidencias): estado vacío de buen-estado con ícono de check en círculo verde tenue, "Sin incidencias — todo va bien".
DATOS DE EJEMPLO (ilustrativos): tipos "Destinatario ausente", "Reagendado"; pedidos id corto; tiempos 2h, 6h.
NOTA DE CONVERGENCIA: badges de incidencia del set; estado vacío de buen-estado en verde como confianza. Densidad relajada del portal; header del portal idéntico al resto.
