...
VISTA: `/conductor/manifiesto/[pedidoId]` — Detalle de una parada (PWA)
DISPOSITIVO: móvil vertical 9:19
CONTEXTO: Detalle de una entrega para el conductor: dirección con enlace a Google Maps, teléfono del destinatario, instrucciones e incidencia si la hay. Solo lectura, texto grande, legible en movimiento.
...
- Enlace gris con flecha "← Volver al manifiesto" (objetivo de toque alto).
- Encabezado: nombre del destinatario muy grande (bold ~24px) a la izquierda y badge de estado grande a la derecha (azul "En ruta").
- Tarjeta de dirección: ícono de pin, dirección en negrita y comuna en gris; debajo, un botón ancho azul (info) "Abrir en Google Maps" con ícono de pin.
- Tarjeta de teléfono (si hay): ícono de teléfono, etiqueta "Teléfono del destinatario" y el número grande como enlace navy táctil.
- Recuadro azul tenue "Instrucciones de entrega" con el texto.
- Recuadro ámbar tenue (si hay incidencia abierta): ícono de alerta, "Hay una incidencia abierta: Destinatario ausente" y nota gris "Si tienes información nueva, comenta con tu coordinador."
DATOS DE EJEMPLO (ilustrativos): destinatario "Joaquín Tapia"; dirección "Av. Apoquindo 4501, Las Condes"; teléfono +56 9 8765 4321; instrucciones "Dejar en conserjería".
NOTA DE CONVERGENCIA: botones y enlaces táctiles grandes (≥44px), recuadros azul (info) y ámbar (advertencia) del set, badges del set. Cabecera del conductor idéntica a las otras vistas de la PWA.
