...
VISTA: `/conductor` — Home del conductor (PWA)
DISPOSITIVO: móvil vertical 9:19
CONTEXTO: Punto de entrada de la PWA del conductor en su teléfono. En la práctica redirige al manifiesto del día (su ruta). Representa la pantalla mínima y táctil que ve el conductor al abrir la app.
ESTRUCTURA (PWA del conductor, idéntica en todas las vistas `conductor/`): pantalla de teléfono vertical; cabecera mínima blanca fija con, a la izquierda, "Mis entregas", y a la derecha una navegación táctil con dos enlaces grandes "Manifiesto" y "Liquidaciones" + un botón "Cerrar sesión". Contenido en una columna angosta (ancho máx ~512px) con objetivos de toque grandes (≥44px), contraste alto y tipografía grande, sobre fondo claro.
LAYOUT del contenido (equivalente al manifiesto del día):
- Encabezado: nombre del manifiesto (bold ~20px) y línea gris "14/06/2026 — 12 pedidos para hoy".
- Banner permanente azul (info) con ícono: "Para registrar la entrega, usa la app de **Mercado Envíos Flex**. Esta app es solo de referencia." (sin botón de cerrar).
- Lista vertical de tarjetas de parada grandes: cada tarjeta con número de orden grande gris arriba-izquierda, badge de estado arriba-derecha, nombre del destinatario grande, dirección y comuna en gris, y (si hay) un recuadro ámbar tenue "Incidencia: …".
- Al pie, botón primario navy de ancho completo "Listo para salir".
DATOS DE EJEMPLO (ilustrativos): manifiesto "Ruta Oriente — 14/06"; paradas "Joaquín Tapia" (Las Condes), "Antonia Reyes" (Ñuñoa).
NOTA DE CONVERGENCIA: estética táctil, grande y de alto contraste; mismos badges de estado y navy del set, pero todo en formato teléfono. La cabecera del conductor debe verse idéntica en las 4 vistas de la PWA.
