...
VISTA: `/conductor/manifiesto` — Manifiesto del día (PWA)
DISPOSITIVO: móvil vertical 9:19
CONTEXTO: La pantalla que el conductor más usa: su ruta del día como lista de paradas ordenadas, con un banner permanente que recuerda usar la app de Flex y un botón "Listo para salir". Solo lectura (sin captura de POD).
ESTRUCTURA (PWA del conductor, idéntica en todas las vistas `conductor/`): teléfono vertical; cabecera mínima blanca fija con "Mis entregas" a la izquierda y nav táctil "Manifiesto" / "Liquidaciones" + "Cerrar sesión" a la derecha. Contenido en columna angosta (~512px), táctil, contraste alto, tipografía grande.
LAYOUT del contenido:
- Encabezado: nombre del manifiesto (bold ~20px) y línea gris "14/06/2026 — **12 pedidos para hoy**".
- Banner permanente azul (info) con ícono de información: "Para registrar la entrega, usa la app de **Mercado Envíos Flex**. Esta app es solo de referencia." (no es cerrable ni colapsable).
- Lista vertical numerada de **tarjetas de parada** grandes, una por pedido: número de orden grande y gris en la esquina superior izquierda; badge de estado en la esquina superior derecha (verde "Entregado", azul "En ruta", ámbar "Pendiente de asignación"); nombre del destinatario grande en negrita; dirección y comuna en gris; instrucciones en cursiva si existen; recuadro ámbar tenue con ícono de alerta "Incidencia: Destinatario ausente" cuando aplica (borde de la tarjeta en ámbar).
- Botón primario navy de **ancho completo y grande** al pie: "Listo para salir".
DATOS DE EJEMPLO (ilustrativos): manifiesto "Ruta Oriente — 14/06"; paradas "Joaquín Tapia" — Av. Apoquindo 4501, Las Condes; "Antonia Reyes" — Irarrázaval 1200, Ñuñoa; "Ignacio Pérez" — Pedro de Valdivia 230, Providencia.
NOTA DE CONVERGENCIA: tarjetas grandes táctiles, badges de estado del set, banner azul (info) permanente, botón navy ancho. Cabecera del conductor idéntica a las otras vistas de la PWA.
