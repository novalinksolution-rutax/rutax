...
VISTA: `/manifiestos/nuevo` — Crear manifiesto
DISPOSITIVO: escritorio 16:10
CONTEXTO: Formulario corto para crear un manifiesto en borrador; luego se le agregan pedidos.
ESTRUCTURA (backoffice del courier, idéntica a las demás `(tenant)`): barra lateral izquierda blanca con marca "Flex Couriers SpA" y navegación agrupada (Dashboard; OPERACIÓN: Pedidos/Manifiestos/Incidencias; DINERO; CONFIGURACIÓN); ítem activo "Manifiestos". Barra superior con campana de avisos y nombre de usuario. Contenido centrado sobre gris claro, pero el formulario va en una columna angosta centrada (~512px).
LAYOUT del contenido:
- Título "Nuevo manifiesto" (bold ~24px) y subtítulo gris: "El manifiesto se crea en borrador. Podrás agregar pedidos antes de confirmarlo."
- Formulario en una columna dentro (campos con etiqueta arriba): selector "Conductor" (estilo shadcn); campo "Nombre del manifiesto" (ej. "Ruta Oriente — 14/06"); campo de fecha "Fecha de operación" (precargada con hoy); área opcional "Notas".
- Botón primario navy "Crear manifiesto" y botón secundario "Cancelar" a su izquierda.
DATOS DE EJEMPLO (ilustrativos): conductor "Diego Soto"; nombre "Ruta Oriente — 14/06"; fecha 14/06/2026.
NOTA DE CONVERGENCIA: formulario de una columna con inputs del set (etiqueta arriba, foco navy); selector estilo shadcn (no nativo). Shell idéntico al resto del backoffice.
