...
VISTA: `/manifiestos/[id]/asignar` — Asignar pedidos al conductor
DISPOSITIVO: escritorio 16:10
CONTEXTO: Selector de pedidos pendientes para agregar a un manifiesto en borrador, con selección múltiple (checkboxes) y una barra de acción fija al pie. Flujo de asignación masiva.
ESTRUCTURA (backoffice del courier, idéntica a las demás `(tenant)`): barra lateral izquierda blanca con marca "Flex Couriers SpA" y navegación agrupada (Dashboard; OPERACIÓN: Pedidos/Manifiestos/Incidencias; DINERO; CONFIGURACIÓN); ítem activo "Manifiestos". Barra superior con campana de avisos y nombre de usuario. Contenido centrado máx ~1150px sobre gris claro.
LAYOUT del contenido:
- Enlace gris "← Volver al manifiesto".
- Encabezado: título "Agregar pedidos" (bold ~24px) y línea gris "Manifiesto: Ruta Oriente — 14/06".
- Barra de filtros: selector "Seller" y campo "Comuna".
- Lista de pedidos disponibles como filas seleccionables con **checkbox** a la izquierda; cada fila muestra destinatario (negrita) + dirección/comuna, badge de estado (ámbar "Pendiente de asignación") y, si un pedido ya está asignado a otro, una advertencia tenue ("Ya asignado a Francisca Vera — Ruta Sur").
- Barra de acción FIJA al pie del contenido (sticky), blanca con borde superior: a la izquierda "3 pedidos seleccionados", a la derecha botón primario navy "Agregar al manifiesto".
DATOS DE EJEMPLO (ilustrativos): destinatarios "Valentina Muñoz" (Maipú), "Camilo Díaz" (Puente Alto), "Antonia Reyes" (Ñuñoa); seller "Tienda Aurora".
NOTA DE CONVERGENCIA: checkboxes estilo shadcn, badges de estado del set, barra de acción masiva sobria. Shell idéntico al resto del backoffice.
