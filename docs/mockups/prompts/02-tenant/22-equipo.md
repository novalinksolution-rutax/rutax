...
VISTA: `/equipo` — Gestión de equipo y roles
DISPOSITIVO: escritorio 16:10
CONTEXTO: Quién tiene acceso a la cuenta del courier, con qué rol, y qué invitaciones siguen pendientes — una sola vista con usuarios activos e invitaciones.
ESTRUCTURA (backoffice del courier, idéntica a las demás `(tenant)`): barra lateral izquierda blanca con marca "Flex Couriers SpA" y navegación agrupada (Dashboard; OPERACIÓN; DINERO; CONFIGURACIÓN: Configuración/Equipo/Sellers/Exportar datos); ítem activo "Equipo". Barra superior con campana de avisos y nombre de usuario "Camila Rojas". Contenido centrado (~896px) sobre gris claro.
LAYOUT del contenido:
- Encabezado: título "Equipo" (semibold ~24px) y subtítulo gris "Quién tiene acceso a tu cuenta, con qué rol, y qué invitaciones siguen pendientes."; a la derecha botón primario navy "Invitar a alguien".
- Fila de pestañas/filtro: "Miembros activos" (activa), "Invitaciones pendientes".
- Tabla/lista en tarjeta. Columnas: **Persona** (avatar inicial circular + nombre en negrita + correo gris debajo), **Rol** (badge gris/navy tenue: "Dueño", "Supervisor", "Coordinador", "Administración"), **Estado** (badge verde "Activo" o ámbar "Invitado"), y a la derecha menú kebab (cambiar rol / revocar).
DATOS DE EJEMPLO (ilustrativos): "Camila Rojas" (camila.rojas@flexcouriers.cl, Dueño, Activo), "Diego Soto" (Supervisor, Activo), "Matías Fuentes" (Coordinador, Invitado), "Paula Herrera" (Administración, Activo).
NOTA DE CONVERGENCIA: badges de rol y estado del set; avatar de inicial sobrio. Misma tabla y shell del backoffice.
