...
VISTA: `/operaciones/[pedidoId]` — Detalle de pedido
DISPOSITIVO: escritorio 16:10
CONTEXTO: Ficha de un pedido: estado, trazabilidad del dinero, historial de estados, incidencias, asignación y acciones según rol. Vista de un coordinador/dueño.
ESTRUCTURA (backoffice del courier, idéntica a las demás `(tenant)`): barra lateral izquierda fija blanca con marca "Flex Couriers SpA" y navegación agrupada (Dashboard; OPERACIÓN: Pedidos/Manifiestos/Incidencias; DINERO: Períodos/Liquidaciones/Conciliación/Pagos; CONFIGURACIÓN: Configuración/Equipo/Sellers/Exportar datos); ítem activo "Pedidos". Barra superior con campana de avisos y nombre de usuario. Contenido centrado máx ~1150px sobre gris claro.
LAYOUT del contenido:
- Enlace gris con flecha "← Volver a pedidos".
- Encabezado: a la izquierda nombre del destinatario (bold ~24px) y dirección+comuna en gris; a la derecha un badge de estado grande (ej. azul "En ruta").
- Lista de definiciones en grilla de 4: Tipo (Flex), Seller (enlace), ID interno (mono pequeño), ML Shipment ID (mono gris).
- Sección "Trazabilidad del dinero": un **trazador del lazo** horizontal estilo línea de tiempo con nodos conectados — Entrega → Línea de cobro → Período → Factura → Pago — cada nodo con un mini-badge de estado (verde/ámbar/gris) y monto en CLP donde aplica.
- Sección "Historial de estados": tarjeta blanca con una lista vertical tipo timeline; cada ítem con un punto navy, el cambio "Asignado → En ruta" y fecha/hora en gris.
- Sección "Incidencias activas" (si hay): tarjeta con borde tenue ámbar/rojo, tipo de incidencia, "Abierta hace 6h" y badge de estado.
- Sección "Asignación": tarjeta con Conductor, Manifiesto y Fecha de operación.
- Sección "Acciones": fila de botones secundarios "Reasignar", "Registrar incidencia", "Cambiar estado", "Descargar etiqueta".
DATOS DE EJEMPLO (ilustrativos): destinatario "Joaquín Tapia", dirección "Av. Apoquindo 4501, Las Condes"; seller "ElectroHogar"; conductor "Diego Soto"; manifiesto "Ruta Oriente — 14/06".
NOTA DE CONVERGENCIA: el trazador del lazo usa los mismos badges/colores de estado del set; timeline con punto navy. Shell, tarjetas y tipografía idénticos al resto del set.
