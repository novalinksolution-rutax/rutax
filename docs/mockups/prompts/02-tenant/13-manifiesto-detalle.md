...
VISTA: `/manifiestos/[id]` — Detalle de manifiesto
DISPOSITIVO: escritorio 16:10
CONTEXTO: Vista de un manifiesto con su estado, los pedidos asignados ordenados por ruta y acciones según estado (agregar pedidos, confirmar, cancelar).
...
- Enlace gris "← Volver a manifiestos".
- Encabezado: a la izquierda nombre del manifiesto (bold ~24px) y línea gris "Conductor: Diego Soto — Fecha: 14/06/2026"; a la derecha un badge de estado grande (ámbar "Borrador").
- Sección "Pedidos asignados (12)": a la derecha botón secundario "+ Agregar pedidos". Tabla dentro de tarjeta (densidad cómoda). Columnas: **#** (orden de ruta, centrado, mono), **Estado** (badge), **Destinatario** (negrita + comuna), **Dirección**, **F. compromiso**, y a la derecha un botón para quitar pedido (solo en borrador).
- Sección de acciones (en borrador): botones "Agregar pedidos", botón primario navy "Confirmar manifiesto" y botón secundario "Cancelar manifiesto".
DATOS DE EJEMPLO (ilustrativos): manifiesto "Ruta Oriente — 14/06"; conductor "Diego Soto"; destinatarios "Joaquín Tapia" (Las Condes), "Antonia Reyes" (Ñuñoa), "Ignacio Pérez" (Providencia).
NOTA DE CONVERGENCIA: misma tabla y badges del set; columna "#" en cifras tabulares. Shell idéntico al resto del backoffice.
