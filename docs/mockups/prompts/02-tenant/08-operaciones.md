...
VISTA: `/operaciones` — Pedidos (panel multi-seller)
DISPOSITIVO: escritorio 16:10
CONTEXTO: El centro de gravedad operativo: lista de pedidos del día con contadores por estado, filtros y tabla densa. Es el flujo más repetido del courier.
ESTRUCTURA (backoffice del courier, idéntica en todas las vistas `(tenant)`): barra lateral izquierda fija (~240px), blanca, marca **"Flex Couriers SpA"** arriba; navegación agrupada: (sin grupo) Dashboard; **OPERACIÓN** → Pedidos, Manifiestos, Incidencias; **DINERO** → Períodos, Liquidaciones, Conciliación, Pagos; **CONFIGURACIÓN** → Configuración, Equipo, Sellers, Exportar datos. Ítem activo "Pedidos" resaltado en navy tenue. Botón "Cerrar sesión" al pie. Barra superior blanca con campana de avisos y nombre "Diego Soto" a la derecha. Contenido centrado máx ~1150px sobre gris claro.
LAYOUT del contenido:
- Encabezado: título "Pedidos" (bold ~24px) a la izquierda; a la derecha, botón secundario "Ver incidencias" y botón primario navy "Nuevo pedido same-day".
- Fila de 5 contadores de estado como tarjetitas con fondo de color tenue y número grande tabular: "Pendiente asig." (ámbar) · "Asignados" (azul) · "En ruta" (azul) · "Entregados" (verde) · "Con problemas" (rojo).
- Barra de filtros: selectores (estilo shadcn, no nativos) "Seller" y "Estado", un campo de fecha, botón "Filtrar"; si hay filtro activo, enlace gris "Limpiar filtros".
- Tabla densa (densidad compacta) dentro de una tarjeta blanca; barra superior de la tabla con conteo gris "148 pedidos". Columnas: **Estado** (badge), **Destinatario** (nombre en negrita + comuna gris debajo), **Seller**, **Fecha comprometida** (a la derecha, mono), **Conductor** (o "Sin asignar" en ámbar), **Tipo** (badge gris "Flex"/"Same-day"), y a la derecha enlace navy "Ver detalle". Filas con líneas finas horizontales y hover sutil.
DATOS DE EJEMPLO (ilustrativos): destinatarios "Valentina Muñoz" (Maipú), "Joaquín Tapia" (Las Condes), "Antonia Reyes" (Ñuñoa); sellers "Tienda Aurora", "ElectroHogar"; estados Entregado/En ruta/Pendiente de asignación.
NOTA DE CONVERGENCIA: misma tabla (encabezado gris, sin líneas verticales, numéricos a la derecha) y mismos badges de estado del set. Shell idéntico al de las demás vistas del backoffice.
