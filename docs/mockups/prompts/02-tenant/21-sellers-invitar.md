...
VISTA: `/sellers/invitar` — Invitar seller
DISPOSITIVO: escritorio 16:10
CONTEXTO: Alta de un nuevo seller (cliente) del courier y envío de su invitación al portal. Formulario corto en una columna.
ESTRUCTURA (backoffice del courier, idéntica a las demás `(tenant)`): barra lateral izquierda blanca con marca "Flex Couriers SpA" y navegación agrupada (Dashboard; OPERACIÓN; DINERO; CONFIGURACIÓN: Configuración/Equipo/Sellers/Exportar datos); ítem activo "Sellers". Barra superior con campana de avisos y nombre de usuario. Contenido en una columna centrada (~640px) sobre gris claro.
LAYOUT del contenido:
- Título "Invitar a un seller" (semibold ~24px) y subtítulo gris: "Registra a tu cliente y le enviaremos una invitación para que entre a su portal y conecte su cuenta de Mercado Libre."
- Formulario en una columna (inputs con etiqueta arriba): "Razón social" (ej. "Boutique Lila SpA"); "RUT" con placeholder `12.345.678-9` y ayuda de formato; "Nombre de contacto"; "Correo electrónico del seller".
- Botón primario navy "Enviar invitación" y botón secundario "Cancelar".
DATOS DE EJEMPLO (ilustrativos): razón social "Boutique Lila SpA"; RUT 76.999.888-7; contacto "Paula Herrera"; correo `paula@boutiquelila.cl`.
NOTA DE CONVERGENCIA: formulario de una columna con inputs del set, validación de RUT chileno implícita. Shell idéntico al resto del backoffice.
