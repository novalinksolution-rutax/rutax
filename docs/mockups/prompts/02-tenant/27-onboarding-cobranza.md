...
VISTA: `/onboarding/cobranza` — Conectar banco para cobranza (Fintoc)
DISPOSITIVO: escritorio 16:10
CONTEXTO: Conecta la cuenta bancaria del courier para reconocer solos los pagos de los sellers y cruzarlos con sus facturas. La conexión se guarda cifrada. Paso opcional del onboarding.
...
- Título "Cobranza a tus sellers" (semibold ~24px) y subtítulo gris: "Conecta tu cuenta bancaria para que reconozcamos solos los pagos que te hacen tus sellers y los crucemos con sus facturas. La conexión se guarda cifrada — nunca verás aquí las credenciales de tu banco."
- Tarjeta blanca central con un ícono de banco/enlace, breve explicación de qué hará la conexión y un botón primario navy "Conectar mi banco" (abre el widget de Fintoc).
- Nota de seguridad gris pequeña con ícono de candado: las credenciales se cifran y nunca se muestran.
- (Estado conectado, referencia): la tarjeta mostraría un badge verde "Banco conectado" y la cuenta enmascarada.
DATOS DE EJEMPLO (ilustrativos): banco "Banco de Chile"; cuenta corriente terminada en •• 4501; empresa "Flex Couriers SpA".
NOTA DE CONVERGENCIA: tratamiento de secreto (candado, "se guarda cifrado"), badge verde "conectado" del set. Shell y tarjeta idénticos al backoffice.
