...
VISTA: `/onboarding/dte` — Facturación electrónica (proveedor DTE + certificado)
DISPOSITIVO: escritorio 16:10
CONTEXTO: Configuración del proveedor de DTE, carga del certificado digital y credenciales del courier. Todo se cifra; una vez cargado, no se vuelve a mostrar, solo a reemplazar.
ESTRUCTURA (backoffice del courier, idéntica a las demás `(tenant)`): barra lateral izquierda blanca con marca "Flex Couriers SpA" y navegación agrupada; ítem activo "Configuración". Barra superior con campana de avisos y nombre de usuario. Contenido en una columna centrada (~640px) sobre gris claro.
LAYOUT del contenido:
- Título "Facturación electrónica" (semibold ~24px) y subtítulo gris: "Elige tu proveedor de DTE y carga tu certificado digital y tus credenciales. Todo se cifra antes de guardarse — una vez cargado, no podrás volver a verlo aquí, solo reemplazarlo."
- Formulario en una columna, en tarjeta blanca:
  - Selector "Proveedor de DTE" (estilo shadcn) con opción "SimpleFactura (sandbox)".
  - Campo de carga de "Certificado digital (.pfx)" marcado como sensible (ícono de candado, nota gris "Se guarda cifrado"); muestra un estado tipo "Cargado ✓" en verde tenue cuando ya existe.
  - Campo "Contraseña del certificado" (enmascarado, sensible).
  - Campos de credenciales del proveedor (enmascarados).
- Botón primario navy "Guardar" con texto de estado "Guardado · 14:32" cuando corresponde.
DATOS DE EJEMPLO (ilustrativos): proveedor "SimpleFactura (sandbox)"; empresa "Flex Couriers SpA".
NOTA DE CONVERGENCIA: campos sensibles con tratamiento visual de secreto (candado, "se guarda cifrado"); formulario de una columna con inputs del set. Shell idéntico al backoffice.
