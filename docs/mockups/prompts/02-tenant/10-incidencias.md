...
VISTA: `/operaciones/incidencias` — Bandeja de incidencias
DISPOSITIVO: escritorio 16:10
CONTEXTO: Vista consolidada de las incidencias del courier (ausente, dirección, reagendado, etc.), con filtros y gestión por fila. La usa supervisión/coordinación.
ESTRUCTURA (backoffice del courier, idéntica a las demás `(tenant)`): barra lateral izquierda blanca con marca "Flex Couriers SpA" y navegación agrupada (Dashboard; OPERACIÓN: Pedidos/Manifiestos/Incidencias; DINERO; CONFIGURACIÓN); ítem activo "Incidencias". Barra superior con campana de avisos y nombre de usuario. Contenido centrado máx ~1150px sobre gris claro.
LAYOUT del contenido:
- Encabezado: enlace gris "← Pedidos" seguido del título "Incidencias" (bold ~24px).
- Barra de filtros: selectores "Seller", "Tipo", "Estado" (por defecto "Abiertas + en gestión"), campo "Desde fecha", botón primario navy "Filtrar" y enlace gris "Limpiar".
- Tabla dentro de tarjeta blanca. Columnas: **Estado** (badge: rojo "Abierta", ámbar "En gestión", verde "Resuelta"), **Tipo** (texto en negrita, ej. "Destinatario ausente"), **Pedido** (id mono corto, enlace), **Seller**, **Abierta hace** (ej. "6h", y si supera 4h, en rojo con "(sin gestión)"), y a la derecha un botón de acción/menú kebab para gestionar la incidencia.
DATOS DE EJEMPLO (ilustrativos): tipos "Destinatario ausente", "Dirección incorrecta", "Reagendado"; sellers "Tienda Aurora", "ElectroHogar"; tiempos 2h, 6h, 9h.
NOTA DE CONVERGENCIA: badges de estado de incidencia exactamente con el mapa de color del set (rojo abierta, ámbar en gestión, verde resuelta, gris cerrada). Misma tabla y shell del resto del backoffice.
