...
VISTA: `/configuracion/exportar-datos` — Exportar datos del courier
DISPOSITIVO: escritorio 16:10
CONTEXTO: Portabilidad de datos: descarga una copia (JSON) de los datos de la cuenta del courier. No incluye credenciales ni tokens. Reservada a dueño/administración.
ESTRUCTURA (backoffice del courier, idéntica a las demás `(tenant)`): barra lateral izquierda blanca con marca "Flex Couriers SpA" y navegación agrupada (Dashboard; OPERACIÓN; DINERO; CONFIGURACIÓN: Configuración/Equipo/Sellers/Exportar datos); ítem activo "Exportar datos". Barra superior con campana de avisos y nombre de usuario. Contenido en una columna centrada (~640px) sobre gris claro.
LAYOUT del contenido:
- Título "Exportar datos" (semibold ~24px) y subtítulo gris: "Descarga una copia de los datos de tu cuenta en formato JSON: sellers, conductores, pedidos, manifiestos, incidencias, períodos de cobro, líneas de cobro, liquidaciones y documentos tributarios. Esto NO incluye credenciales ni tokens de conexión."
- Tarjeta blanca: "Archivo de exportación (JSON)", una línea gris explicando que la descarga puede tardar y que queda registrada en la bitácora de auditoría, y un botón primario navy con ícono de descarga "Descargar mis datos".
DATOS DE EJEMPLO (ilustrativos): empresa "Flex Couriers SpA".
NOTA DE CONVERGENCIA: una sola tarjeta sobria, un único botón primario navy con ícono de descarga. Shell idéntico al resto del backoffice.
