...
VISTA: `/onboarding` — Configuración / panel de activación del courier
DISPOSITIVO: escritorio 16:10
CONTEXTO: Centro de mando del onboarding: un checklist persistente (no bloqueante) que muestra qué falta para dejar el courier listo para operar — facturación electrónica (DTE), folios CAF y tarifas, con cobranza como paso opcional.
ESTRUCTURA (backoffice del courier, idéntica a las demás `(tenant)`): barra lateral izquierda blanca con marca "Flex Couriers SpA" y navegación agrupada (Dashboard; OPERACIÓN; DINERO; CONFIGURACIÓN: Configuración/Equipo/Sellers/Exportar datos); ítem activo "Configuración". Barra superior con campana de avisos y nombre de usuario. Contenido centrado máx ~1150px sobre gris claro. Bajo la barra superior puede verse una franja-banner de onboarding con barra de progreso ("2 de 3 pasos completados").
LAYOUT del contenido:
- Título "Configura tu cuenta" (semibold ~24px) con una **barra de progreso** y texto "2 de 3 pasos completados".
- Lista vertical de tarjetas-paso, cada una con: un círculo de estado a la izquierda (verde con check = completado; navy tenue numerado = pendiente), título del paso, descripción gris del "por qué", y a la derecha un botón ("Configurar" / "Listo ✓"):
  - Paso "Facturación electrónica (DTE)" — completado (check verde).
  - Paso "Folios CAF" — completado (check verde).
  - Paso "Tarifas" — pendiente (botón navy "Configurar").
  - Paso opcional "Cobranza (Fintoc)" — marcado como "Opcional", botón secundario "Conectar".
DATOS DE EJEMPLO (ilustrativos): empresa "Flex Couriers SpA".
NOTA DE CONVERGENCIA: checklist como secuencia guiada sobria; check verde y numeración navy tenue con los colores del set; barra de progreso navy. Shell idéntico al resto del backoffice.
