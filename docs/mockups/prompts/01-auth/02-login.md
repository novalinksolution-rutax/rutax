...
VISTA: `/login` — Inicio de sesión (usuarios internos del courier)
DISPOSITIVO: escritorio, formulario centrado (16:10)
CONTEXTO: Pantalla de acceso de los roles internos del courier (dueño, supervisor, coordinador, administración). El mismo formulario sirve también al seller. Tras ingresar, el sistema redirige a cada quien a su área.
LAYOUT (de arriba hacia abajo):
- Pantalla completa en gris muy claro, sin navegación. Una sola tarjeta blanca centrada (~360px de ancho), borde 1px, sombra mínima, esquinas ~10px.
- Wordmark **Rutax** arriba. Título "Iniciar sesión" (semibold ~24px). Subtítulo gris: "Ingresa con tu correo y contraseña".
- Campo "Correo electrónico": etiqueta arriba, input con ícono de sobre a la izquierda, placeholder `tu@correo.cl`.
- Campo "Contraseña": etiqueta arriba, input con ícono de candado a la izquierda, valor enmascarado `••••••••`.
- Botón primario navy de ancho completo "Ingresar".
DATOS DE EJEMPLO (ilustrativos): empresa "Flex Couriers SpA"; correo `diego.soto@flexcouriers.cl`.
NOTA DE CONVERGENCIA: idéntico lenguaje visual a la vista de entrada `/` y al login del seller — misma tarjeta, mismos inputs con ícono, mismo botón navy. Mantén EXACTAMENTE la misma tipografía y paleta del resto del set.
