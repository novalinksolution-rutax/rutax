...
VISTA: `/registro/revisa-tu-correo` — Revisa tu correo (estado intermedio)
DISPOSITIVO: escritorio, formulario centrado (16:10)
CONTEXTO: Pantalla de confirmación tras el alta de la empresa. Sin acción posible más que esperar el correo de activación; comunica la expectativa y la vigencia del enlace.
LAYOUT (de arriba hacia abajo):
- Fondo gris muy claro. Tarjeta blanca centrada (~448px), CENTRADA EN TEXTO, borde 1px.
- Arriba, un círculo de fondo navy muy tenue con un ícono de sobre en navy.
- Título "Revisa tu correo" (~20px). Descripción gris: "Enviamos un enlace a **camila.rojas@flexcouriers.cl** para que actives tu cuenta y crees tu contraseña. El enlace vence en 7 días." (el correo en peso medio y color de texto principal).
- Botón secundario (borde) "Reenviar correo".
- Línea fina gris al pie: "¿Necesitas ayuda? Contacta a soporte." (con "Contacta a soporte" subrayado).
DATOS DE EJEMPLO (ilustrativos): correo `camila.rojas@flexcouriers.cl`.
NOTA DE CONVERGENCIA: misma tarjeta centrada, mismo patrón de ícono en círculo navy-tenue que las pantallas de activación/invitación. Mantén EXACTAMENTE tipografía y paleta del set.
