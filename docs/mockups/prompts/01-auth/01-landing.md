...
VISTA: `/` — Entrada / landing de la aplicación
DISPOSITIVO: escritorio, formulario centrado (16:10)
CONTEXTO: Punto de entrada del producto. En la práctica es una redirección al inicio de sesión: quien no tiene sesión activa cae aquí, en la pantalla de login centrada de Rutax. Representa la "puerta" de la app para los tres tipos de usuario (courier interno, seller, conductor).
LAYOUT (de arriba hacia abajo):
- Pantalla completa en gris muy claro, sin barra de navegación. Una sola tarjeta blanca centrada vertical y horizontalmente, de ancho acotado (~360px), borde 1px y sombra mínima, esquinas ~10px.
- Dentro de la tarjeta: la marca **Rutax** arriba (wordmark sobrio en navy o casi-negro, sin logotipo recargado), luego título "Iniciar sesión" (semibold, ~24px) y subtítulo gris "Ingresa con tu correo y contraseña".
- Campo "Correo electrónico" con etiqueta arriba, input con ícono de sobre a la izquierda y placeholder `tu@correo.cl`.
- Campo "Contraseña" con etiqueta arriba, input con ícono de candado a la izquierda y placeholder de puntos `••••••••`.
- Botón primario navy ancho completo "Ingresar".
- Pie discreto opcional con enlace gris "¿Olvidaste tu contraseña?".
DATOS DE EJEMPLO (ilustrativos): correo `camila.rojas@flexcouriers.cl`.
NOTA DE CONVERGENCIA: mantén EXACTAMENTE la misma tipografía, el mismo navy de marca, el mismo estilo de tarjeta blanca con borde sutil, inputs y botón primario que en todas las demás vistas del set. Sin imágenes de marketing ni fondos con color.
