...
VISTA: `/invitacion/[token]` — Aceptar invitación al equipo
DISPOSITIVO: escritorio, formulario centrado (16:10)
CONTEXTO: Pantalla donde una persona invitada (equipo interno, seller o conductor) acepta su invitación. Variante mostrada: persona nueva que define su contraseña para unirse al courier que la invitó.
LAYOUT (de arriba hacia abajo):
- Fondo gris muy claro. Tarjeta blanca centrada (~448px), borde 1px.
- Encabezado centrado: círculo de fondo navy muy tenue con ícono de candado navy; título "Define tu contraseña" (~20px); descripción gris: "Flex Couriers SpA te invitó como coordinador de tráfico. Crea tu contraseña para empezar."
- Formulario en una columna: campo "Tu nombre completo"; campo "Contraseña" (enmascarado) con **medidor de fortaleza** de cuatro segmentos (verde/ámbar/gris) y etiqueta "Buena"; campo "Confirma tu contraseña".
- Botón primario navy ancho completo "Crear contraseña y entrar".
- (Variante alternativa, solo referencia: si la persona ya tiene cuenta, la tarjeta muestra un ícono de check navy, título "Confirma para aceptar" y un único botón "Aceptar e ingresar".)
DATOS DE EJEMPLO (ilustrativos): empresa "Flex Couriers SpA"; invitado "Matías Fuentes", rol "coordinador de tráfico".
NOTA DE CONVERGENCIA: idéntica al patrón de `/activar-cuenta` (mismo encabezado de ícono en círculo navy-tenue, mismo medidor de fortaleza, mismo botón navy). Mantén EXACTAMENTE tipografía y paleta del set.
