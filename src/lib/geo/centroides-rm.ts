/**
 * Centroides aproximados de las 52 comunas de la Región Metropolitana.
 * =====================================================================
 *
 * Referencia geográfica compartida. Vive en `src/lib/geo/` —y no dentro de un
 * módulo— porque lo consumen tanto `integraciones` (adaptador stub de
 * geocoding) como `contexto` (Torre de control). Si viviera dentro de un
 * módulo de negocio, un adaptador de `integraciones` tendría que importar de
 * él, que es el sentido de flecha que el proyecto prohíbe.
 *
 * QUÉ SON: centroides aproximados a 3–4 decimales (error del orden de cientos
 * de metros). NO son un resultado de geocoding ni una fuente de precisión.
 *
 * USOS LEGÍTIMOS
 *   - Punto de consulta meteorológica y de calidad del aire por comuna, en
 *     PRODUCCIÓN. Open-Meteo acepta múltiples coordenadas en una sola llamada,
 *     así que las 52 comunas se resuelven con 1 request por ciclo. El error de
 *     cientos de metros es irrelevante: la grilla del modelo es de kilómetros.
 *   - Coordenada determinista del adaptador stub de geocoding (dev/CI), que
 *     necesita un lat/long plausible sin tocar la red ni costar dinero.
 *
 * USO PROHIBIDO
 *   - Presentar un centroide como la ubicación de un pedido. La ubicación de un
 *     pedido sale del geocoding y solo del geocoding; el estado autoritativo es
 *     `operacion.pedidos.geo_estado`, no la existencia de un lat/long. Rellenar
 *     con el centroide de la comuna borraría el contador de "pedidos sin
 *     ubicar", que la Torre de control está obligada a mostrar siempre.
 *   - Calcular el centro visual de una zona. Ése es el centroide del polígono
 *     disuelto de sus comunas, no el promedio de estos puntos.
 *
 * Las claves son los nombres EXACTOS del catálogo `COMUNAS_RM`, y el test de
 * este módulo verifica que ambos no se desincronicen.
 */
...
export const CENTROIDES_RM: Record<ComunaRM, { lat: number; long: number }> = {
  ...
  'Pedro Aguirre Cerda': { lat: -33.4869, long: -70.6733 },
