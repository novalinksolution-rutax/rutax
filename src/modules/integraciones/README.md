...
- `contexto/` — puertos de contexto externo de la Torre de control: `clima/` (Open-Meteo Forecast), `aire/` (Open-Meteo Air Quality) y `calendario/` (feriados de api.boostr.cl + cálculo determinístico de la restricción vehicular GEC, que NO se consulta a ninguna API). Los tres son datos GLOBALES, no por tenant, y ninguno usa credenciales en su tier libre. Contrato propio de degradación en `contexto/resultado.ts`: **estos puertos no lanzan ante un fallo de la fuente** — devuelven `ResultadoContexto`, que sabe decir "no pude", para que el job marque la capa como degradada en `contexto.fuentes_estado` en vez de caerse. ⚠️ El tier gratuito de Open-Meteo NO permite uso comercial; ver la nota en `contexto/open-meteo-comun.ts` y en `.env.example`.
