...
- `falabella/` — adaptador de Falabella Seller Center (tercera fuente de pedidos). **Hoy existe solo el cimiento**: `firma.ts` (firmador HMAC-SHA256 de la query string, función pura, validado contra el vector oficial de la doc) y `cliente-http.ts` (`peticionFalabella` sobre `reintentarConBackoff`). Todavía NO hay puerto ni `index.ts`, así que tampoco hay superficie pública: **nada fuera de este directorio debe importar `cliente-http.ts`** — cuando llegue la ingesta, se importará desde su `index.ts` como en `ml/` y `shopify/`. Tres cosas que este proveedor NO comparte con los otros: **no tiene sandbox** (toda llamada golpea producción del seller), su credencial es `UserID` (público, viaja en cada URL) + **API Key** (secreto, es la clave de firma y nunca sale de `firma.ts`), y un error suyo puede llegar con HTTP 200 dentro de un sobre `ErrorResponse`. Detalle de lo verificado contra la doc oficial, con fecha, en la cabecera de cada archivo.
...
- `contexto/` — puerto de contexto externo de la Torre de control: queda **uno**, `calendario/` (feriados de api.boostr.cl + cálculo determinístico de la restricción vehicular GEC, que NO se consulta a ninguna API). Es dato GLOBAL, no por tenant, y no usa credenciales en su tier libre. Contrato propio de degradación en `contexto/resultado.ts`: **este puerto no lanza ante un fallo de la fuente** — devuelve `ResultadoContexto`, que sabe decir "no pude", para que el job marque la fuente como degradada en `contexto.fuentes_estado` en vez de caerse. ⚠️ Los puertos `clima/` y `aire/` **se retiraron el 2026-08-03**: el rediseño v2 de la Torre sacó esas dos capas del producto entero (ver `docs/torre-de-control/alcance-v2.md` §3), y con ellos cayeron los adaptadores de OpenWeather y la grilla de 14 puntos de la RM. *Memoria por si el tema vuelve: el tier gratuito de Open-Meteo NO permite uso comercial —define como comercial una app con suscripciones—, que fue el motivo de migrar a OpenWeather en su momento.*
