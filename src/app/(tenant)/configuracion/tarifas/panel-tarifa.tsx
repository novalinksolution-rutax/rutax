...
export function PanelTarifa({ sellers, tarifa, trigger }: Props) {
  ...
  const hayMontos = Number.isFinite(nCobras) && Number.isFinite(nPagas) && cobras !== "" && pagas !== "";
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>{trigger ?? <Button size="sm">Nueva tarifa</Button>}</SheetTrigger>

      <SheetContent
        side="right"
        // 430 px es la medida del tablero. `sm:max-w-none` anula el `max-w-sm`
        // que el componente trae por defecto.
        // ⚠️ El `!` no es pereza: `SheetContent` trae
        // `data-[side=right]:sm:max-w-sm` en su clase base, y un selector con
        // atributo de datos gana por especificidad a una utilidad suelta. Sin
        // esto el panel se queda en los 384 px del componente.
        className="w-full gap-0 overflow-y-auto p-0 sm:w-[430px] sm:max-w-[430px]!"
      >
        {/* La cabecera nombra el objeto y su estado. La regla de 2 px es la
            misma marca de sección que usan los correos y la ficha. */}
        <SheetHeader className="gap-1 border-b-2 border-fg px-4 py-3.5">
          <SheetTitle className="text-base font-semibold">
            {esEdicion ? "Editar tarifa" : "Nueva tarifa"}
          </SheetTitle>
          <SheetDescription className="rx-num font-mono text-[11.5px] text-fg-muted">
            {esEdicion
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4 px-4 py-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-1.5">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value={SELLER_DEFECTO}>Todos · por defecto</SelectItem>
                    {sellers.map((s) => (
          ...
          <div className="border border-line bg-bg-inset p-3.5">
            ...
            {hayMontos && (
              <BloqueComposicion
                ...
                total={{ concepto: "de margen por entrega", monto: nCobras - nPagas }}
              />
            )}

            <p className="mt-2.5 text-xs text-fg-subtle">
              Sin IVA — el 19 % se agrega al facturar. Al conductor se le paga por entrega efectiva.
            </p>
          </div>
          ...
          */}
          {margenInvertido && (
            <p className="border border-attention-line bg-attention-bg px-3 py-2 text-sm text-attention-fg">
              ...
              más de lo que le cobras al seller. Cada entrega con esta tarifa te deja esa diferencia
              en contra. Si es a propósito, sigue.
            </p>
          )}
          ...
          <div className="space-y-3 border border-line bg-bg-inset p-3.5">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="recargo_reprogramacion_clp">Recargo si reagendan</Label>
                <Input
